...
const analyzeSelectors = (html: string): SelectorGuess => {
  ...
  const containerSelectors = [
    ...
    '.sds-comps-vertical-layout.fds-ugc-single-intention-item-list',
    '.sds-comps-vertical-layout[data-template-id="layout"][data-template-type="vertical"]',
  ];
  ...
  let intentionItem = '';
  const itemSelectors = [
    '.xYjt3uiECoJ0o6Pj0xOU',
    '.sds-comps-vertical-layout.KuzouLxTqWJ5UVpERiYP',
  ...
  if (!intentionItem) {
    const elements = $('div')
      .filter((i, el) => {
        const $el = $(el);
        const classes = $el.attr('class') || '';
        return classes.includes('item') && !$el.find(intentionItem || '*').length;
      })
      .slice(0, 5);
  ...
  const headlineSelectors = [
    ...
    '.sds-comps-text.sds-comps-text-type-headline1',
    '.title_class',
    '.headline',
    '.title-text',
    'span',
...
const updateParserFile = (selectors: SelectorGuess) => {
  ...
  const updatedContent = content
    .replace(/singleIntentionList:\s*'(.*?)',/, `singleIntentionList: '${selectors.singleIntentionList}',`)
    .replace(/intentionItem:\s*'(.*?)',/, `intentionItem: '${selectors.intentionItem}',`)
    .replace(/intentionTitle:\s*'(.*?)',/, `intentionTitle: '${selectors.intentionTitle}',`)
    .replace(/intentionHeadline:\s*'(.*?)',/, `intentionHeadline: '${selectors.intentionHeadline}',`)
    .replace(/intentionProfile:\s*'(.*?)',/, `intentionProfile: '${selectors.intentionProfile}',`);
