...
pnpm exposure:suite             # 패키지~카페 전체 빠른 병렬 실행
...
`전체 빠른 노출체크` 패널에서는 패키지·일반건·도그마루·루트·애견·서리펫·카페를
선택하고, 대상별 병렬 수(1~8), 최대 페이지(1~9), 동시에 돌릴 대상 수(1~3)를 조절한다.
기본값은 대상별 8개·동시 대상 2개이며, 최대 동시 요청은 16개다.
...
| `EXPOSURE_CONCURRENCY` | 선택 | 대상별/개별 노출체크 병렬 수(기본·최대 8) |
| `EXPOSURE_MAX_PAGES` / `PAGE_CHECK_MAX_PAGES` | 선택 | 확인할 최대 검색 페이지 수 |
| `EXPOSURE_TARGET_CONCURRENCY` | 선택 | 전체 실행에서 동시에 처리할 대상 수(기본 2, 최대 3) |
...
- 정상 응답 구간의 목표 시간은 5분이다. 네이버 403/차단이 발생하면 정확한 결과와 재시도를 우선하므로 5분을 넘을 수 있으며, 대시보드 로그에서 실제 소요 시간과 실패 대상을 확인해야 한다.
- 전체 빠른 실행은 비로그인 교차확인을 1회 시도하되 403 발생 시 60초 재대기하지 않고 다음 키워드로 진행한다. 단독 실행은 기존 2회 재시도를 유지한다.
- 전체 빠른 노출체크는 `SIGINT`/`SIGTERM`에서 하위 프로세스 그룹까지 정리한다. 일부 레거시 단독 엔트리포인트는 별도 신호 핸들러가 없다.
- Keyword/RootKeyword가 이중 스키마로 존재해서 한쪽 스키마를 변경하면 다른 쪽도 수동으로 맞춰야 한다.
- Railway가 현재 공개 운영 경로다. EC2 설정은 복구·병행 운영용으로 유지한다.
