---
description: 네이버 검색 결과 크롤링 자동화로 6시간 수동 작업을 10분으로 단축한 모니터링 시스템
thumbnail: /images/portfolio/naver-blog-monitoring-thumb.jpg
github: https://github.com/ganggyunggyu/blog-cron-bot
featured: true
period: 2024.03 ~ 현재
role: 1인 개발
---
...
하지만 네이버가 `rra` 변형 HTML도 사용하는 것을 발견했다. rra 셀렉터 추가:
...
데스크톱 HTML 구조가 너무 자주 변경되어 유지보수가 힘들었다. 모바일 검색으로 전환했다.
...
초기에는 모든 로직이 `index.ts` 한 파일에 몰려 있었다. 코드가 길어지면서 유지보수가 어려워졌고, FSD 구조로 전면 리팩토링했다.
...
네이버 HTML 구조 변경 시 자동으로 새 셀렉터를 찾는 머신러닝 기반 시스템 도입 검토. 현재는 수동 도구가 있는데, 이걸 자동화하면 유지보수 시간을 더 줄일 수 있을 것 같다.
