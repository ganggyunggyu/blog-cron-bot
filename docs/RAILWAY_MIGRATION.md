...
**주의**: `PORT`는 Railway가 자동으로 주입하니 직접 설정하지 않는다. `ecosystem.railway.cjs`가 이 값을 읽어서 대시보드 포트로 씀.
...
EC2 해지 후에는 레포에서 `ecosystem.config.cjs`, `ecosystem.jobs.cjs`(EC2 전용)를 지우고 `ecosystem.railway.cjs`만 남겨도 됨.
